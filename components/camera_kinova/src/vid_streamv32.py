...
class StreamCapture(mp.Process):
    ...
    def run(self):
        ...
        if is_depth:
            pipe = 'rtspsrc name=m_rtspsrc ! rtpgstdepay ! videoconvert ! appsink name=m_appsink'
        else:
            pipe = 'rtspsrc name=m_rtspsrc ! rtph264depay name=m_rtph264depay ! avdec_h264 name=m_avdech264 ! videoconvert name=m_videoconvert ! videorate name=m_videorate ! appsink name=m_appsink'
